...
export default function ProfilePage({ user, profile, isDemo, onProfileUpdated, onLogout, setActiveTab }) {
  ...
  const [saldoAdded, setSaldoAdded] = useState(false);
  ...
  const handleTopUpChange = (e) => {
    const raw = e.target.value.replace(/\./g, '');
    if (raw === '' || /^\d+$/.test(raw)) {
      setTopUpAmount(raw === '' ? 0 : Number(raw));
    }
  };

  const handleQuickAdd = (amt) => {
    setTopUpAmount((prev) => Number(prev) + amt);
  };

  const handleAddSaldo = async () => {
    if (!topUpAmount || Number(topUpAmount) <= 0) return;
    setAddingSaldo(true);
    const newLimit = Number(monthlyLimit) + Number(topUpAmount);
    setMonthlyLimit(newLimit);
    await onProfileUpdated({ monthly_limit: newLimit });
    setTopUpAmount(0);
    setAddingSaldo(false);
    setSaldoAdded(true);
    setTimeout(() => setSaldoAdded(false), 2500);
  };
            ...
            <div className="card" style={styles.settingCard}>
              <div style={styles.settingHeader}>
                ...
                <div>
                  ...
                  <p style={styles.settingDesc}>Limit pengeluaran bulananmu saat ini</p>
                </div>
              </div>

              {/* Current budget — read only display */}
              <div style={styles.currentBudgetBox}>
                <span style={styles.currentBudgetLabel}>Budget Saat Ini</span>
                <span style={styles.currentBudgetValue}>
                  Rp {Number(monthlyLimit).toLocaleString('id-ID')}
                </span>
              </div>

              <div style={styles.topUpDivider} />

              {/* Add Saldo */}
              <div style={styles.topUpHeader}>
                <PlusCircle size={16} color="var(--color-primary)" />
                <span style={styles.topUpTitle}>Tambah Saldo</span>
              </div>
              <div style={styles.budgetInputWrap}>
                ...
                <input
                  type="text"
                  inputMode="numeric"
                  value={topUpAmount === 0 ? '' : formatNumber(topUpAmount)}
                  placeholder="0"
              ...
              <div style={{ ...styles.quickBudgets, marginBottom: '14px' }}>
              ...
              <button
                ...
                disabled={!topUpAmount || addingSaldo}
                className="btn btn-primary"
                style={{
                  ...styles.addSaldoBtn,
                  opacity: (!topUpAmount || addingSaldo) ? 0.6 : 1,
                  cursor: (!topUpAmount || addingSaldo) ? 'not-allowed' : 'pointer',
                }}
              >
                {addingSaldo ? (
                  <span>Menambahkan...</span>
                ) : saldoAdded ? (
                  <><Check size={16} /> Saldo Ditambahkan!</>
                ) : (
...
const styles = {
  ...
  currentBudgetLabel: {
    ...
    letterSpacing: '0.5px', color: 'var(--color-primary-dark)',
  },
  currentBudgetValue: {
  ...
  topUpTitle: { fontSize: '13px', fontWeight: '700', color: 'var(--text-main)' },
  addSaldoBtn: {
